...
To run this practical you need to have an RStudio project as discussed in the 
...
spatial data, merge them with the accessibility indicator we want to plot and then
we plot these data.
...
metrics for specific points and we will use these points as representatives for each
Local Authority. 
...
package can be found [here](https://github.com/riatelab/osrm).
...
```{r eval=FALSE}
df <- data.table::as.data.table(df) # The cumulative_cutoff function is sensitive to NAs in the data and even though you can't see them directly, S3 class of the dataframe might be hiding some in the background information about the variables. Transformation to data.table is advised.
...
::: callout-tip
What do we miss by focusing on London's accessibility?
:::
